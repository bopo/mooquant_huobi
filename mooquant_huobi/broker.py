...
class LiveBroker(broker.Broker):
    ...
    def _onUserTrades(self, trades):
        ...
        for trade in trades:
            ...
            if order is not None:
                ...
                logger.info(
                    'newTrade: price:%f btc:%f fee:%s time:%s' %
    ...
    def submitOrder(self, order):
        if order.isInitial():
            ...
            if order.isBuy():
                provider.bitstampOrder = self.__httpClient.buyLimit(
                    order.getLimitPrice(), order.getQuantity())
            else:
                provider.bitstampOrder = self.__httpClient.sellLimit(
                    order.getLimitPrice(), order.getQuantity())
            ...
            # the order is not yet mapped to the position and
            # Position.onOrderUpdated will get called.
