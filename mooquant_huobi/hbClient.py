...
class hbTradeClient(TradeClientBase):
    ...
    def getAccountBalance(self):
        balances = self.__client.get(
            '/v1/account/accounts/%s/balance' %
            self.__accountid)
        acc = hbAccountBalance(self.__coinType, balances)
        logger.info(
            'getAccountBalance: usdt:%s coin:%s' %
            (acc.getCash(), acc.getCoin()))
    ...
    def cancelOrder(self, orderId):
        ...
        self.checkOrderState(
            orderId, [
                hbOrderState.OrderCanceled, hbOrderState.OrderFilled])
