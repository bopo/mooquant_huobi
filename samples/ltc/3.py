...
#                self.__position = self.marketOrder(self.__instrument, self.__shares)
        ...
        # Check if we have to exit the position.
            ...
            # cross.cross_below(self.__sma[10], self.__sma[30]) > 0:
            self.__position.exitMarket()
...
#    broker_brk = broker.backtesting.Broker(50000, feed)
    ...
    plt.getInstrumentSubplot(coin).addDataSeries(
        "SMA60", myStrategy.getSMA(60))
    plt.getInstrumentSubplot(coin).addDataSeries(
        "SMA10", myStrategy.getSMA(10))
    plt.getInstrumentSubplot(coin).addDataSeries(
        "SMA30", myStrategy.getSMA(30))
